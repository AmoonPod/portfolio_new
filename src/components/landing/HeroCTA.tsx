...
export type HeroCTAVariant = 'baseline' | 'trust' | 'action' | 'lowFriction'
...
const CTA_CONFIG = {
    baseline: {
        ...
        ariaLabel: 'Prenota una call gratuita su WhatsApp per richiedere la tua landing page'
    },
    trust: {
        text: 'Richiedi Preventivo Gratuito',
        ariaLabel: 'Richiedi un preventivo gratuito senza impegno'
    },
    action: {
        text: 'Voglio la Mia Landing',
        ariaLabel: 'Richiedi subito la tua landing page professionale'
    },
    lowFriction: {
        text: 'Inizia Ora - Paga Dopo',
        ariaLabel: 'Inizia il progetto ora e paga alla consegna'
    }
}

export function HeroCTA({ variant, whatsappLink }: HeroCTAProps) {
...
export function HeroCTALarge({ variant, whatsappLink }: HeroCTAProps) {
    const config = CTA_CONFIG[variant]
